...
public:
	enum SendFlags {
		SendBlocking = 1 << 0, SendNonBlocking = 1 << 1, SendBulkOnly = 1 << 2
	};
...
public:
	...
	GetCS(const Environments &envs, const std::string &f,
			...
			Msg(MsgType::GET_CS), versions(envs), filename(f), lang(_lang), count(
					_count), target(_target), arg_flags(_arg_flags), client_id(
					0), preferred_host(host), minimal_host_version(
					_minimal_host_version) {
	}
...
public:
	...
	UseCS(std::string platform, std::string host, unsigned int p,
			...
			Msg(MsgType::USE_CS), job_id(id), hostname(host), port(p), host_platform(
					platform), got_env(gotit), client_id(_client_id), matched_job_id(
					matched_host_jobs) {
	}
...
public:
	CompileFile(CompileJob *j, bool delete_job = false) :
			Msg(MsgType::COMPILE_FILE), deleteit(delete_job), job(j) {
	}
...
public:
	FileChunk(unsigned char *_buffer, size_t _len) :
			Msg(MsgType::FILE_CHUNK), buffer(_buffer), len(_len), del_buf(false) {
	}

	FileChunk() :
			Msg(MsgType::FILE_CHUNK), buffer(0), len(0), del_buf(true) {
	}
...
public:
	CompileResult() :
			Msg(MsgType::COMPILE_RESULT), status(0), was_out_of_memory(false), have_dwo_file(
					false) {
	}
...
public:
	...
	JobBegin(unsigned int j) :
			Msg(MsgType::JOB_BEGIN), job_id(j), stime(time(0)) {
	}
...
public:
	JobLocalBegin(int job_id = 0, const std::string &file = "") :
			Msg(MsgType::JOB_LOCAL_BEGIN), outfile(file), stime(time(0)), id(
					job_id) {
	}
...
public:
	ConfCS() :
			Msg(MsgType::CS_CONF), max_scheduler_pong(MAX_SCHEDULER_PONG), max_scheduler_ping(
					MAX_SCHEDULER_PING) {
	}
...
public:
	...
	EnvTransfer(const std::string &_target, const std::string &_name) :
			Msg(MsgType::TRANFER_ENV), name(_name), target(_target) {
	}
...
public:
	...
	MonGetCS(int jobid, int hostid, GetCS *m) :
			GetCS(Environments(), m->filename, m->lang, 1, m->target, 0,
					std::string(), false), job_id(jobid), clientid(hostid) {
		type = MsgType::MON_GET_CS;
	}
...
public:
	MonJobBegin() :
			Msg(MsgType::MON_JOB_BEGIN), job_id(0), stime(0), hostid(0) {
	}

	MonJobBegin(unsigned int id, unsigned int time, int _hostid) :
			Msg(MsgType::MON_JOB_BEGIN), job_id(id), stime(time), hostid(
					_hostid) {
	}
...
public:
	...
	MonLocalJobBegin(unsigned int id, const std::string &_file,
			...
			Msg(MsgType::MON_LOCAL_JOB_BEGIN), job_id(id), stime(time), hostid(
					_hostid), file(_file) {
	}
...
public:
	...
	MonStats(int id, const std::string &msg_stat) :
			Msg(MsgType::MON_STATS), hostid(id), statmsg(msg_stat) {
	}
...
public:
	...
	StatusText(const std::string &_text) :
			Msg(MsgType::STATUS_TEXT), text(_text) {
	}
...
public:
	...
	VerifyEnv(const std::string &_target, const std::string &_environment) :
			Msg(MsgType::VERIFY_ENV), environment(_environment), target(_target) {
	}
...
public:
	...
	VerifyEnvResult(bool _ok) :
			Msg(MsgType::VERIFY_ENV_RESULT), ok(_ok) {
	}
...
public:
	...
	BlacklistHostEnv(const std::string &_target,
			const std::string &_environment, const std::string &_hostname) :
			Msg(MsgType::BLACKLIST_HOST_ENV), environment(_environment), target(
