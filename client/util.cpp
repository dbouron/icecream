 ...
 **/
        static bool dcc_open_lockfile(const std::string &fname, int &plockfd)
        {
            ...
            if (plockfd == -1 && errno != EEXIST) {
                log_error() << "failed to creat " << fname << ": " << strerror(errno) << std::endl;
                return false;
            }
        ...
        bool dcc_lock_host(int &lock_fd)
        {
            ...
            case EACCES: /* HP-UX and Cygwin give this for exclusion */
                ...
                break;
            default:
                log_error() << "lock " << fname << " failed: " << strerror(errno) << std::endl;
                break;
            }
        ...
        void colorify_output(const std::string &_s_ccout)
        {
